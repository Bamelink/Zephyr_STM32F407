...
const struct device *fcan = DEVICE_DT_GET(DT_ALIAS(fcan));
const struct device *dlcan = DEVICE_DT_GET(DT_ALIAS(dlcan));
...
void rx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	filter_id = can_add_rx_filter_msgq(dlcan, &counter_msgq, &filter);
...
void poll_state_thread(void *unused1, void *unused2, void *unused3)
{
	...
	while (1) {
		err = can_get_state(dlcan, &state, &err_cnt);
		if (err != 0) {
...
void state_change_work_handler(struct k_work *work)
{
	...
	if (current_state == CAN_BUS_OFF) {
		...
		if (can_recover(dlcan, K_MSEC(100)) != 0) {
...
void main(void)
{
	...
	ret = can_add_rx_filter_msgq(dlcan, &change_led_msgq, &change_led_filter);
	if (ret == -ENOSPC) {
	...
	can_set_state_change_callback(dlcan, state_change_callback, &state_change_work);
	...
	while (1) {
		...
		can_send(dlcan, &change_led_frame, K_FOREVER,
			 tx_irq_callback,
			 "LED change");
		k_sleep(SLEEP_TIME);
		...
		can_send(dlcan, &counter_frame, K_MSEC(100), NULL, NULL);
		k_sleep(SLEEP_TIME);
	}
}